...
| Configuration | Environment variables via `dotenvy`, `lazy_static` |
...
src/
  ...
  stream/
    mod.rs                       # Re-exports ido and pool sub-modules
    ido/
...
2. `handle_ws_connection` splits the socket into a read half and a write half.
3. A dedicated sink task reads from an `mpsc::channel<String>(256)` and forwards messages to the WebSocket write half.
4. The read loop parses incoming JSON-RPC requests, dispatches them via `rpc::dispatch`, and sends responses through the same outbound channel.
5. On disconnect (client close frame or read error), `ConnectionState::cleanup_all()` aborts all subscription forwarding tasks, the outbound channel is dropped, and the sink task exits.
...
Avalanche C-Chain
       ...
       v
  stream::ido::stream / stream::pool::stream
       |
       | (decode log -> serde_json::Value)
       v
  stream::ido::receive / stream::pool::receive
       |
       | (EventProducers.{trade,price,project,milestone,new_content}.publish())
       v
...
There are no explicit unsubscribe methods. Subscriptions are cleaned up when:
...
The price channel infrastructure is wired (producer exists in `EventProducers`) but no stream currently publishes price events. The `PriceCache` is initialized and passed to both stream modules, but is not yet used to publish price updates.
...
Two smart contracts are monitored via WebSocket RPC log subscriptions:
...
1. Convert the HTTP RPC URL to a WebSocket URL (`https://` -> `wss://`, `http://` -> `ws://`).
...
When a subscriber lags behind and misses messages (buffer overflow), the forwarding task logs a warning and continues:

```rust
Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
    tracing::warn!(channel = %channel_key, lagged = n, "Subscriber lagged, skipping messages");
}
```
...
| `subscribe` | `fn subscribe(&mut self, key: String, handle: JoinHandle<()>)` | Registers a task; aborts any existing task for the same key |
...
The server handles WebSocket `Ping` and `Pong` frames by ignoring them (continuing the read loop). It does not initiate pings. Native WebSocket-level ping/pong from the client is passed through by Axum.
...
- **Broadcast lag:** Logged as warning, missed messages are dropped, subscription continues.
- **Broadcast channel closed:** Forwarding task exits gracefully.

### Graceful Shutdown

The server listens for `CTRL+C` (via `tokio::signal::ctrl_c`) and performs a graceful shutdown through `axum::serve(...).with_graceful_shutdown(shutdown_signal())`.
...
All lookups are case-insensitive (addresses are lowercased before storage and retrieval). The cache is initialized and passed to both stream modules but is not yet actively used for publishing price events.
